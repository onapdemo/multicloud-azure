...
# show execution
@app.route(route_base + "<vim_id>/stacks/<execution_id>", methods=['GET'])
@auto.doc()
@aria.pass_model_storage
@aria.pass_logger
def show_execution(vim_id, execution_id, model_storage, logger):
    """
...
# install service
@app.route(route_base + "<vim_id>/multipart/createStack/<service_name>",
           methods=['POST'])
@auto.doc()
@aria.pass_logger
def deploySerivce(vim_id, service_name, logger):
    service_template_name = service_name + "-template" + \
                            time.strftime('%Y%m%d%H%M%S')
    status = install_template_private(service_template_name)
    if (status[1] != 200):
        logger.error("Error while installing the service-template")
        return status[0], status[1]
    logger.info("service template {0} valiadated and stored.creating service"
                .format(service_template_name))
    status = create_service(status[2], service_name +
                            time.strftime('%Y%m%d%H%M%S'))
    if (status[1] != 200):
        logger.error('Error while creating the service {0}'
                     .format(status[2].name))
        return status[0], status[1]
    logger.info("Starting Install workflow..")
    return start_execution(status[2].id, 'install')
...
def install_template_private(service_template_name, model_storage,
    ...
    if "multipart/form-data" in request.headers['Content-Type']:
        ...
        f = tempfile.NamedTemporaryFile(prefix="ariatmp_", dir="/tmp/aria",
                                        suffix=fileType,
                                        delete=False)
